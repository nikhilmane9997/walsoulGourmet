...
class ShopByCityContainer extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      unitQty: {},
  ...
  componentDidMount() {
    ...
    if(this.props.homePageData.length !== 0)
    {
      ...
      this.setState({
        allBlogsData:this.props.homePageData[0].response.categoryList.header.top_menu[4].child_data
     });
    }
  }
        ...
        if(_get(nextProps, 'homePageData[0].status') === 200)
         {
             ...
             this.setState({
                  allBlogsData:nextProps.homePageData[0].response.categoryList.header.top_menu[4].child_data
               });
         }
        }
    ...
    return (
      <div>
        <div className="container" style={{paddingTop:'125px'}}>
           ...
           <br/>
           <div className="row">
              {this.state.allBlogsData !== undefined && this.state.allBlogsData.map((contact) => (
              <div className="col-md-3">
                <p><a style={{cursor:'pointer',color:'blue'}} onClick={() => this.getData(contact)}><center>{contact.name}</center></a></p>
              </div>
              ))}
            </div>
        </div>
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(ErrorHandler(ShopByCityContainer));
